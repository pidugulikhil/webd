...
```mermaid
flowchart LR	A[OpenClaw or App] --> B[WEBD Local Gateway]	B --> C[Ollama API<br/>50000]	B --> D[OpenAI API<br/>50001]	C --> E[Browser Automation]	D --> E	E --> F[chatgpt.com]	E --> G[claude.ai]
```
...
-   Provider: Ollama
-   Base URL: [http://localhost:50000](http://localhost:50000)
-   Model examples:
    -   chatgpt
    -   claude
...
```bash
curl http://localhost:50000/api/versioncurl http://localhost:50001/v1/models
```

## Feature Snapshot

```mermaid
pie title WEBD Focus Areas	"Session Reuse & Recovery" : 35	"API Compatibility" : 30	"Streaming & UX Stability" : 20	"Docs & Beginner Experience" : 15
```
...
```markdown
![Preview 1](assets/photo1.png)![Preview 2](assets/photo2.png)
```

## Quick Troubleshooting

1.  Old response repeats: restart WEBD and retry once.
2.  Message typed but not sent: WEBD now returns explicit send-failed error; retry same prompt.
3.  Curl JSON body errors: use examples from [CURL_COMMANDS.md](CURL_COMMANDS.md).
